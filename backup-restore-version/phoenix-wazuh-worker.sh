...
# Phoenix Wazuh Worker Script
# This script runs ON the Phoenix Unraid server to deploy Docker containers
# Uses pre-edited docker-compose.yml and handles certificate generation
...
if [ "$MODE" = "restore" ]; then
    echo "  Mode: RESTORE (backup: $BACKUP_TIMESTAMP)"
else
...
echo "🔑 Step 4.5: Restoring agent identities from backup (if restore mode)..."

if [ "$MODE" = "restore" ] && [ -n "$BACKUP_TIMESTAMP" ]; then
    BACKUP_PATH="/mnt/user/wazuh-data/backups/$BACKUP_TIMESTAMP"

    echo "Restoring agent identities from backup: $BACKUP_TIMESTAMP"

    # Restore client.keys (agent credentials)
    if [ -f "$BACKUP_PATH/client-keys/client.keys" ]; then
        cp "$BACKUP_PATH/client-keys/client.keys" volumes/wazuh_etc/client.keys
        chown root:999 volumes/wazuh_etc/client.keys
        chmod 640 volumes/wazuh_etc/client.keys
        echo "✅ Restored client.keys"
    fi

    # Restore sslmanager.key (manager SSL private key)
    if [ -f "$BACKUP_PATH/client-keys/sslmanager.key" ]; then
        cp "$BACKUP_PATH/client-keys/sslmanager.key" volumes/wazuh_etc/sslmanager.key
        chown root:999 volumes/wazuh_etc/sslmanager.key
        chmod 640 volumes/wazuh_etc/sslmanager.key
        echo "✅ Restored sslmanager.key"
    fi

    # Restore sslmanager.cert (manager SSL certificate)
    if [ -f "$BACKUP_PATH/client-keys/sslmanager.cert" ]; then
        cp "$BACKUP_PATH/client-keys/sslmanager.cert" volumes/wazuh_etc/sslmanager.cert
        chown root:999 volumes/wazuh_etc/sslmanager.cert
        chmod 640 volumes/wazuh_etc/sslmanager.cert
        echo "✅ Restored sslmanager.cert"
    fi

    echo ""
    AGENT_COUNT=$(wc -l < "$BACKUP_PATH/client-keys/client.keys" 2>/dev/null || echo "0")
    echo "✅ Agent identities restored from backup - $AGENT_COUNT agents will reconnect"
    echo "   Agents will reconnect automatically with preserved IDs and names"
    echo "   ℹ️  Agent database will recreate automatically (Wazuh design)"
else
    echo "⏩ SKIPPED - Fresh install mode"
    echo "   Agents will enroll as new devices on first connection"
fi
echo ""
...
INDEXER_TIMEOUT=300  # 5 minutes - generous timeout, hitting it means real failure
